...
requires_openai_auth = true`
  ...
  const codexAuthJson = `{
  "OPENAI_API_KEY": "${firstKey || 'YOUR_API_KEY'}"
}`

  const claudeSettingsJson = `{
  "env": {
    "ANTHROPIC_BASE_URL": "${baseUrl}",
    "ANTHROPIC_AUTH_TOKEN": "${firstKey || 'YOUR_API_KEY'}",
    "CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC": "1"
  }
}`

  const claudeEnvUnix = `export ANTHROPIC_BASE_URL="${baseUrl}"
export ANTHROPIC_AUTH_TOKEN="${firstKey || 'YOUR_API_KEY'}"
export CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`

  const claudeEnvWindows = `set ANTHROPIC_BASE_URL=${baseUrl}
set ANTHROPIC_AUTH_TOKEN=${firstKey || 'YOUR_API_KEY'}
set CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`

  const responsesCurl = `curl -X POST ${baseUrl}/v1/responses \\
  -H "Authorization: Bearer ${firstKey || 'YOUR_API_KEY'}" \\
  ...
  -H "x-api-key: ${firstKey || 'YOUR_API_KEY'}" \\
  -H "Content-Type: application/json" \\
        ...
        <div className="min-w-0">
          ...
          <Card id="qs-curl" className="mb-4 scroll-mt-20 py-0">
            <CardContent className="p-6">
              ...
              <p className="mb-4 text-sm text-muted-foreground">{t('docs.quickStart.curlDesc')}</p>
              <div className="space-y-4">
                <div>
                  <h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">/v1/responses</h4>
                  <CodeBlock label="cURL" content={responsesCurl} lang="bash" />
                </div>
                <div>
                  <h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">/v1/chat/completions</h4>
                  <CodeBlock label="cURL" content={chatCurl} lang="bash" />
                </div>
                <div>
                  <h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">/v1/messages</h4>
                  <CodeBlock label="cURL" content={messagesCurl} lang="bash" />
                </div>
              </div>
            </CardContent>
          </Card>
