...
func (h *Handler) TestConnection(c *gin.Context) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		case http.StatusUnauthorized:
			h.store.ApplyAccountHardFailure(account, 24*time.Hour, "unauthorized")
		case http.StatusTooManyRequests:
			h.store.ApplyModelCooldown(account, testModel, time.Now().Add(5*time.Minute), "rate_limited")
		}
	...
	for _, account := range accounts {
		...
		go func(acc *auth.Account) {
			...
			case http.StatusUnauthorized:
				...
				h.store.ApplyAccountHardFailure(acc, 24*time.Hour, "unauthorized")
				atomic.AddInt64(&bannedCount, 1)
			case http.StatusTooManyRequests:
				if usagePct, ok := proxy.ParseCodexUsageHeaders(resp, acc); ok {
					h.store.PersistUsageSnapshot(acc, usagePct)
				}
				h.store.ApplyModelCooldown(acc, testModel, time.Now().Add(5*time.Minute), "rate_limited")
