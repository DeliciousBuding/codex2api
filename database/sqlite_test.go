...
func TestSoftDeleteAccountReturnsNoRowsForMissingAccount(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "codex2api.db")
	...
	if err := db.SoftDeleteAccount(context.Background(), 404); err != sql.ErrNoRows {
		t.Fatalf("SoftDeleteAccount missing account error = %v, want sql.ErrNoRows", err)
	}
}
